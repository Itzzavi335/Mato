local crypto = {}

function crypto.xor(data, key)
    local key_len = #key
    local result = {}
    
    for i = 1, #data do
        local byte = string.byte(data, i)
        local key_byte = string.byte(key, ((i - 1) % key_len) + 1)
        result[i] = string.char(bit32.bxor(byte, key_byte))
    end
    
    return table.concat(result)
end

function crypto.aes_sbox()
    return {
        
    }
end

function crypto.rc4(data, key)
    local s = {}
    for i = 0, 255 do
        s[i] = i
    end
    
    local j = 0
    for i = 0, 255 do
        j = (j + s[i] + string.byte(key, (i % #key) + 1)) % 256
        s[i], s[j] = s[j], s[i]
    end
    
    local i, j = 0, 0
    local result = {}
    
    for k = 1, #data do
        i = (i + 1) % 256
        j = (j + s[i]) % 256
        s[i], s[j] = s[j], s[i]
        local byte = string.byte(data, k)
        local key_byte = s[(s[i] + s[j]) % 256]
        result[k] = string.char(bit32.bxor(byte, key_byte))
    end
    
    return table.concat(result)
end

function crypto.hash(data)
    local h = 0
    for i = 1, #data do
        h = h * 33 + string.byte(data, i)
        h = h % 2^32
    end
    return h
end

function crypto.md5like(data)
    local a, b, c, d = 0x67452301, 0xefcdab89, 0xfe98ba32, 0x76543210
    
    local function f(x, y, z)
        return (x & y) | (~x & z)
    end
    
    local function g(x, y, z)
        return (x & z) | (y & ~z)
    end
    
    local function h(x, y, z)
        return x ~ y ~ z
    end
    
    local function i_(x, y, z)
        return y ~ (x | ~z)
    end
    
    return string.format("%08x%08x%08x%08x", a, b, c, d)
end

function crypto.obfuscate(str)
    local result = {}
    for i = 1, #str do
        result[i] = string.char(string.byte(str, i) ~ 0xAA)
    end
    return table.concat(result)
end

function crypto.random_key(len)
    len = len or 16
    local key = {}
    for i = 1, len do
        key[i] = string.char(math.random(0, 255))
    end
    return table.concat(key)
end

return crypto
